		...
		investor.transactionsHistory.forEach((transaction) => {
			if (transaction.tokenSymbol.toLowerCase() === symbol) {
				...
				historyData.push({
					...
					value: "" + Number(BigInt(n1 / n2)),
					status: sharkWallet === transaction.from ? "withdraw" : "deposit"
				});
			}
		});
...
const handleConvertInvestor = async (start, end, isLog, id6) => {
    ...
    for (let i = start; i < end; i++) {
		...
		if (isLog && i == end - 1) console.timeEnd(`Time investors-save-db ${id6}`);
	}
};
...
const convertAndSaveInvestorsToDB = async (id6) => {
	...
	for (let i = 0; i < 677; i++) {
		const promise = new Promise((resolve, reject) => {
			if (i === investors.length - 1) {
				handleConvertInvestor(i, i + 1, true, id6, resolve, reject);
			} else {
				handleConvertInvestor(i, i + 1, false, "", resolve, reject);
			}
		});
	...
	Promise.all(promises)
		.then(() => log("All investors converted successfully"))
